...
func init() {
	...
	Register(&types.Rule{
		...
		GetNewCommand: specific.SudoRewrite(func(c *types.Command) []string {
			...
			if strings.Contains(c.Output, "Usage:") && len(c.ScriptParts()) > 2 {
				return utils.ReplaceCommand(c, c.ScriptParts()[2], cmds)
			}
			m := dockerNotRe.FindStringSubmatch(c.Output)
			if m == nil {
				return nil
			}
			return utils.ReplaceCommand(c, m[1], cmds)
		}),
	})
	...
	Register(&types.Rule{
		...
		GetNewCommand: func(c *types.Command) []string {
			...
			closest := utils.GetClosest(parts[1], getGolangCmds(), 0.6, true)
	...
	Register(&types.Rule{
		...
		GetNewCommand: func(c *types.Command) []string {
			...
			return utils.ReplaceCommand(c, strings.TrimSpace(m[1]), getGemCmds())
		},
	})
